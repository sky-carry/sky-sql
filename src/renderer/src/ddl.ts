...
function parseColumnType(columnType: string): { type: string; length: string; scale: string } {
  // 基础类型名可含数字（datetime2 等）
  const m = /^([a-zA-Z_][a-zA-Z_0-9 ]*?)\s*\(\s*(\d+)\s*(?:,\s*(\d+)\s*)?\)\s*(.*)$/.exec(columnType.trim())
...
/** 表名（PG / SQL Server 支持 schema.name，MySQL 加库名前缀） */
function quoteTable(dbType: DbType, database: string, name: string): string {
  ...
  if (dbType === 'postgresql' || dbType === 'sqlserver') {
    const defaultSchema = dbType === 'postgresql' ? '"public"' : '[dbo]'
    const idx = name.indexOf('.')
    return idx === -1
      ? `${defaultSchema}.${q(name)}`
      : `${q(name.slice(0, idx))}.${q(name.slice(idx + 1))}`
  }
  return q(name)
}
...
function columnDef(dbType: DbType, c: DesignColumn, opts: { inlinePk?: boolean } = {}): string {
  ...
  // SQL Server 的 IDENTITY 紧跟类型，在 NULL 约束之前
  if (dbType === 'sqlserver' && c.autoIncrement && isIntType(c.type)) {
  ...
  let def = formatDefault(c.defaultValue)
  // SQL Server 没有 TRUE/FALSE 字面量
  if (dbType === 'sqlserver' && def) def = def.replace(/^TRUE$/i, '1').replace(/^FALSE$/i, '0')
...
export function generateCreateTable(dbType: DbType, database: string, d: TableDesign): string[] {
  ...
  // PG / SQLite / SQL Server 的二级索引独立建
  if (dbType === 'postgresql' || dbType === 'sqlite' || dbType === 'sqlserver') {
    for (const i of d.indexes.filter((x) => x.name && x.columns.length)) {
      // SQL Server 的索引方法（CLUSTERED/NONCLUSTERED）放在 INDEX 关键字前
      const msMethod = dbType === 'sqlserver' && i.method ? `${i.method} ` : ''
      statements.push(
        `CREATE ${i.unique ? 'UNIQUE ' : ''}${msMethod}INDEX ${q(i.name)} ON ${table}` +
...
export function generateAlterTable(
  ...
  if (dbType === 'sqlserver') {
    ...
    for (const i of [...droppedIdx, ...changedIdx]) {
      statements.push(`DROP INDEX ${q(i.originalName ?? i.name)} ON ${table}`)
    }
    for (const c of droppedCols) {
      statements.push(`ALTER TABLE ${table} DROP COLUMN ${q(c.originalName ?? c.name)}`)
    }
    for (const c of keptCols) {
      const o = origByName.get(c.originalName!)!
      if (fullType(o).toLowerCase() !== fullType(c).toLowerCase() || o.notNull !== c.notNull) {
        statements.push(
          `ALTER TABLE ${table} ALTER COLUMN ${q(c.originalName!)} ${fullType(c)} ${c.notNull ? 'NOT NULL' : 'NULL'}`
        )
      }
      if ((o.defaultValue ?? '') !== (c.defaultValue ?? '')) {
        if (!o.defaultValue && c.defaultValue) {
          ...
          statements.push(`ALTER TABLE ${table} ADD DEFAULT ${def} FOR ${q(c.originalName!)}`)
        } else {
          warnings.push(
            `-- 警告: SQL Server 的默认值是命名约束，修改/删除列 ${c.name} 的默认值需先手动 DROP 对应约束，该项变更已忽略`
          )
        }
      }
      if ((o.comment ?? '') !== (c.comment ?? '')) {
        warnings.push(`-- 警告: SQL Server 的列注释（扩展属性）暂不支持在设计器中修改，列 ${c.name} 的注释变更已忽略`)
      }
      if (o.autoIncrement !== c.autoIncrement) {
        warnings.push(`-- 警告: SQL Server 不支持直接切换自增（IDENTITY），列 ${c.name} 的该项变更已忽略；如需修改请重建表`)
      }
      if (c.name !== c.originalName) {
        statements.push(`EXEC sp_rename N'${table.replace(/'/g, "''")}.${q(c.originalName!)}', N'${c.name.replace(/'/g, "''")}', N'COLUMN'`)
      }
    }
    for (const c of addedCols) {
      statements.push(`ALTER TABLE ${table} ADD ${columnDef(dbType, c)}`)
    }
    if (pkChanged) {
      if (origPk.length > 0 && original.pkConstraint) {
        statements.push(`ALTER TABLE ${table} DROP CONSTRAINT ${q(original.pkConstraint)}`)
    ...
    for (const i of [...changedIdx, ...addedIdx]) {
      statements.push(
        `CREATE ${i.unique ? 'UNIQUE ' : ''}${i.method ? `${i.method} ` : ''}INDEX ${q(i.name)} ON ${table} (${i.columns.map(q).join(', ')})`
      )
    }
    for (const f of [...changedFk, ...addedFk]) {
      statements.push(`ALTER TABLE ${table} ADD ${fkClause(dbType, database, f)}`)
    ...
    if ((d.comment ?? '') !== (original.design.comment ?? '')) {
      warnings.push('-- 警告: SQL Server 的表注释（扩展属性）暂不支持在设计器中修改，该项变更已忽略')
    }
    if (d.name !== original.name && d.name) {
      const newName = d.name.includes('.') ? d.name.split('.').pop()! : d.name
      statements.push(`EXEC sp_rename N'${table.replace(/'/g, "''")}', N'${newName.replace(/'/g, "''")}'`)
    }
    return [...warnings, ...statements]
  }
